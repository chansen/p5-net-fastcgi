...
foreach my $type (FCGI_PARAMS, FCGI_GET_VALUES, FCGI_GET_VALUES_RESULT) {
    ...
    {
        my $dump = dump_record($type, 1, build_params({ 'x' => 'y' x 128 }));
        like $dump, qr/\A \{ $name\, \s+ 1\, \s "\\001\\200\\000\\000\\200 x y+/x;
    }
    {
        my $dump = dump_record($type, 1, "\001\001");
...
# Streams
{
    ...
    foreach my $test (@tests) {
        ...
        my $dump = dump_record($type, $request_id, $content);
        like $dump, $expected;
    }
}
...
# FCGI_BEGIN_REQUEST
{
    ...
    foreach my $test (@tests) {
        ...
        my $dump = dump_record(FCGI_BEGIN_REQUEST, 1, $content);
        like $dump, $expected;
    }
}
...
# FCGI_END_REQUEST
{
    ...
    foreach my $test (@tests) {
        my ($content, $expected) = @$test;
        my $dump = dump_record(FCGI_END_REQUEST, 1, $content);
        like $dump, $expected;
    }
}
...
# FCGI_UNKNOWN_TYPE
{
    ...
    foreach my $test (@tests) {
        my ($content, $expected) = @$test;
        my $dump = dump_record(FCGI_UNKNOWN_TYPE, 0, $content);
        like $dump, $expected;
    }
}
