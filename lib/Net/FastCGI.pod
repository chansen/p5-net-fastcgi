...
few convenient higher level functions such as build_begin_request(), build_end_request(),
parse_record() and dump_record().

Work has begun on object oriented implementation and a simple blocking IO class which is 
