...
is returned. Otherwise, a false value (C<undef> in scalar context and an empty 
...
holds the error from the C<sysread> call. If C<read_header> encounters 
...
holds the error from the C<sysread> call. If C<read_record> encounters 
end-of-file after some but not all of the needed octets, the function returns 
a false value and sets C<$!> to C<EPIPE>.
...
Determines wheter or not the givien file handle C<$fh> is ready for reading 
...
Upon successful completion, C<0> or C<1>. Otherwise, C<undef> and C<$!> holds 
the C<select> error.
...
Determines wheter or not the givien file handle C<$fh> is ready for writing 
within the given timeout C<$timeout>.
