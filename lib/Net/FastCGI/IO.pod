...
Attempts to read a C<FCGI_Header> from file handle C<$fh>. If C<read_header> 
...
Attempts to read a C<FCGI_Record> from file handle C<$fh>. If C<read_record> 
reaches end-of-file before reading any octets, it returns a false value 
(C<undef> in scalar context and an empty list in list context). If unsuccessful, 
...
C<undef> and C<$!> holds the error from the C<syswrite> call.
