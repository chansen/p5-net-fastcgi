...
Attempts to read a C<FCGI_Header> from filehandle C<$fh>. If C<read_header()> 
reaches end-of-file before reading any octets, it returns a false value 
(C<undef> in scalar context and an empty list in list context). If unsuccessful, 
C<read_header()> returns a false value and C<$!> holds the error from the 
C<sysread()> call. If C<read_header()> encounters end-of-file after some but 
not all of the needed octets, the function returns a false value and sets C<$!> 
to C<EPIPE>. Upon successful completion, the return value of L<Net::FastCGI::Protocol/parse_header> 
is returned.

This function handles partial reads and restarts on C<EINTR>.
...
is returned. Otherwise, in scalar context C<undef> and in list context an empty list.

=head2 read_record

Attempts to read a C<FCGI_Record> from filehandle C<$fh>. If C<read_record()> 
reaches end-of-file before reading any octets, it returns a false value 
(C<undef> in scalar context and an empty list in list context). If unsuccessful, 
C<read_record()> returns a false value and C<$!> holds the error from the 
C<sysread()> call. If C<read_record()> encounters end-of-file after some but 
not all of the needed octets, the function returns a false value and sets C<$!> 
to C<EPIPE>. Upon successful completion, the return value of L<Net::FastCGI::Protocol/parse_record> 
is returned.
