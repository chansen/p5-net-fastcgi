...
A hash reference containing name/value pairs. Typically this is the CGI environ 
that the application expects.

=item C<$stdin> (optional)

A string of octets containing the C<FCGI_STDIN> data. This should only be provided if 
C<$role> is set to either C<FCGI_RESPONDER> or C<FCGI_FILTER>. Maybe C<undef> or empty. 
Stream is terminated if provided.

=item C<$data> (optional)

A string of octets containing the C<FCGI_DATA> data. This should only be provided if 
C<$role> is set to C<FCGI_FILTER>. Maybe C<undef> or empty. Stream is terminated if provided.
...
A string of octets containing the C<FCGI_STDOUT> data. Maybe C<undef> or empty. 
Stream is terminated if provided.

=item C<$stderr> (optional)

A string of octets containing the C<FCGI_STDERR> data. Maybe C<undef> or empty.
Stream is terminated if provided.
...
A string of octets containing the content, cannot exceed 65535 octets in length. May be C<undef>.
...
A string of octets containing the stream content. May be C<undef>.
...
Stream is not terminated if C<$content> is C<undef> or empty unless C<$terminate> is set.
...
A string of octets containing the content. Maybe C<undef>.
...
A hash reference which represents the record. The content depends on the
...
A string of octets containing the content of the stream. Can be empty but not C<undef>.
...
Returns a hash reference containing the C<FCGI_Record> components.
See L</parse_record>.
